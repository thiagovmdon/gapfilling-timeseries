...
#%%
def fillgapslrc(nonfilleddata: pd.pandas.core.frame.DataFrame, coords: pd.pandas.core.frame.DataFrame, filtaux = pd.pandas.core.frame.DataFrame, work_w_filter = True, Use_distance = True, nummaxcorrec = 50, Use_t = True):
...
#%%
def getr2fromlrs(nonfilleddata: pd.pandas.core.frame.DataFrame, coords: pd.pandas.core.frame.DataFrame, filtaux = pd.pandas.core.frame.DataFrame, work_w_filter = True, Use_distance = True, nummaxcorrec = 50, Use_t = True):
    
    '''Function to get the R² from the LR (multiple or single) performed for filling the gaps of time-series (originally rainfall):
    This function is a faster alternative for just obtining the scores, and not filling the time-series. 
    Inputs
    ------------------
    nonfilleddata: dataframe[n x y]: 
        Dataframe with the time-series to be filled. Gaps should be stored as np.nan, 
